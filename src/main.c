...
#define m_encode_move(source, dest, prom) (U16)(source) | (U16)((dest) << 6U) | (U16)((prom) << 12U)
#define m_decode_move(source, dest, prom, move) source = (int)((move) & 0x3fU); dest = (int)(((move) >> 6U) & 0x3fU); prom = (int)(((move) >> 12U) & 0xfU) 
...
void read_fen(Position* position, char* fen){
    int square = 0;
    while(*fen != ' ' && square < 64){
        if((*fen >= 'a' && *fen <= 'z') || (*fen >= 'A' && *fen <= 'Z')){
            m_set_bit(position->bits[char_pieces[(int)(*fen)]], square++);
        }
        else if(*fen >= '1' && *fen <= '8'){
            square += *fen - '0';
        }
...
void print_position(Position position){
    for (int i = 0; i < 8; i++)
    {
        ...
        for (int j = 0; j < 8; j++)
        {
            if(m_get_bit(get_sum_bitposition(&position), i*8 + j)){
                for(int piece = P; piece <= k; piece++)
                {
                    printf("%s", m_get_bit(position.bits[piece], i*8+j) ? unicode_pieces[piece] : "");
                }
            }
            else printf("  ");
        }
    ...
    printf("\nSide to move: %s", position.side == white ? "white" : "black");
}
...
Position get_standard_position(){
    Position position;
    position.bits[p] = 0x000000000000ff00ULL;
    position.bits[P] = 0x00ff000000000000ULL;
    position.bits[k] = 0x0000000000000008ULL;
    position.bits[K] = 0x0800000000000000ULL;
    position.bits[q] = 0x0000000000000010ULL;
    position.bits[Q] = 0x1000000000000000ULL;
    position.bits[r] = 0x0000000000000081ULL;
    position.bits[R] = 0x8100000000000000ULL;
    position.bits[b] = 0x0000000000000024ULL;
    position.bits[B] = 0x2400000000000000ULL;
    position.bits[n] = 0x0000000000000042ULL;
    position.bits[N] = 0x4200000000000000ULL;
    position.side = white;
    return position;
}
    ...
    for(int i = 0; i < knight_count; i++){
        ...
        for(int j = 0; j < candidate_count; j++){
            ...
            if(m_get_bit(get_color_bitposition(position, ally), dest) == 0ULL){
                tab.moves[tab.count++] = m_encode_move(source, dest, empty);
    ...
    for(int i = 0; i < pawn_count; i++){
        ...
        for(int j = 0; j < candidate_count; j++){
            ...
            if(m_get_bit(get_color_bitposition(position, enemy), dest)){
                if(dest < 8 || dest > 55){
                    tab.moves[tab.count++] = m_encode_move(source, dest, N + ally * 6);
                    tab.moves[tab.count++] = m_encode_move(source, dest, B + ally * 6);
                    tab.moves[tab.count++] = m_encode_move(source, dest, R + ally * 6);
                    tab.moves[tab.count++] = m_encode_move(source, dest, Q + ally * 6);
                }
                else tab.moves[tab.count++] = m_encode_move(source, dest, empty);
                //printf("\n%s%s", index_to_coord[source], index_to_coord[dest]);
            }
        }
        //moves
...
static inline int evaluate_position(Position* position){
    ...
    int ally = position->side == white ? white : black;
    int enemy = position->side == white ? black : white;
    score += 200*(get_population_count(position->bits[ally * 6 + K]) - get_population_count(position->bits[enemy * 6 + K]));
...
void make_move(Position* position, U16 move){
    ...
    int dest;
    int prom;
    m_decode_move(source, dest, prom, move);
    for(int i = P; i <= k; i++){
    ...
    for(int i = P; i <= k; i++){
        if(m_get_bit(position->bits[i], source)){
            ...
            if(prom == empty) m_set_bit(position->bits[i], dest);
            else m_set_bit(position->bits[prom], dest);
        }
    }
...
void read_move(U16 word){
    ...
    int dest;
    int prom;
    m_decode_move(source, dest, prom, word);
    printf("\n%s %s %s", index_to_coord[source], index_to_coord[dest], prom == empty ? "" : unicode_pieces[prom]);
}
U16 parse_move(){
    int source;
    int dest;
    ...
    prom = (buff[4] == '\n' || buff[4] == ' ' || buff[4] == 0) ? empty : char_pieces[(int)buff[4]];
    U16 move = m_encode_move(source, dest, prom);
    read_move(move);
    return move;
}
int main(){
    ...
    read_fen(&position, start_position);
    ...
    /*
    read_fen(&position, tricky_position);
    position.bits[P] |= 0x200ULL;
    print_position(position);
    Move_tab tab = move_generator(&position);
    for(int i = 0; i < tab.count; i++){
        read_move(tab.moves[i]);
        Position temp;
        memcpy(&temp, &position, sizeof(Position));
        make_move(&temp, tab.moves[i]);
        print_position(temp);
        printf("\n\n");
    }
    position.side = black;
